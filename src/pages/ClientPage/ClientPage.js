...
export default () => {
  ...
  return (
    <Layout>
      <Quest questBody={questBody} onStartQuest={handleStartQuest} onAnswer={handleAnswer} />
      ...
      {!isReady && <div className="loading-screen"><h1>LOADING</h1></div>}
    </Layout>
  );
}
...
const Quest = (props) => {
  ...
  const { questBody: { gift_photo, gift_description, name, tasks }, onAnswer, onStartQuest } = props;
  ...
  return <div className="">
    ...
    <div className="quest-header">
      ...
      {gift_description && <div className="gift-description">{gift_description}</div>}
      {isQuestFinished && <div className="">YOU WON!!!</div>}
    </div>
...
const TaskType = ({ isActive, question, description, id, type, unlocked, onAnswer, options }) => {

  const [ answer, setAnswer ] = useState('');

  return <div className={`c-task ${isActive ? 'active' : 'inactive'}`}>
...

    {
      ...
      :
        unlocked ?
          <div className="c-done">DONE</div>
        :
          <div className="c-done">LOCKED</div>
    }
